...
import {
  createBookstoreReview,
  fetchBookstoreReviews,
  getReviewStorageMode,
  type ReviewStorageMode,
} from "@/lib/reviews";
...
export default function BookstoreReviews({ bookstoreId, bookstoreName }: BookstoreReviewsProps) {
  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      ...
      try {
        const [nextReviews, nextStorageMode] = await Promise.all([
          fetchBookstoreReviews(bookstoreId),
          getReviewStorageMode(),
        ]);
        if (!cancelled) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const nextStorageMode = await getReviewStorageMode();
    ...
    <section className="mt-5 border-t border-ink-100 pt-5">
      ...
      {storageMode === "local" && (
        <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          리뷰 데이터베이스 테이블이 아직 없어 이 브라우저에만 임시 저장됩니다. Supabase SQL Editor에서
          <code className="mx-1 font-mono">supabase/schema.sql</code>을 실행하거나, 로컬에서{" "}
          <code className="font-mono">npm run setup:reviews</code>를 실행해 주세요.
        </p>
      )}
